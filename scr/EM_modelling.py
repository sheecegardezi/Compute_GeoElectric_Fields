...
from scr import functions_EM_modelling as femm
...
if mode == 1: 
    ...
    mh_obs, h_obs = femm.read_magnetics( 
            in_path, 
            str(obs_f), 
            mag_path, 
            secs_path, 
...
if mode == 2:     
    ...
    comp, std_comp = femm.compute_regional_b_field(
            in_path, 
            str(obs_f), 
            reg_ref, 
            mag_path, 
    ...
    comp_rmf, std_comp_rmf= femm.compute_regional_b_field(
            in_path, 
            str(sit_f), 
            reg_ref, 
            mag_path, 
    ...
    pmh_obs, h_obs = femm.read_magnetics( 
            in_path, 
            str(obs_f), 
            mag_path, 
            secs_path, 
    ...
    mh_obs, dh_obs, std_dh_obs = femm.difference_b_fields(
            pmh_obs, 
            h_obs,             
            comp, 
            std_comp, 
...
# 3.1) Select the fields depending if we are with Approach#1 or Approach #2
if mode == 1:
    fname = "SECS_"
if mode == 2:
    fname = "dSECS_"
...
secs_e_fields = np.zeros([length, 
                          len(e_site),
                          2, 
                          stat])
    
reg_e_fields = np.zeros([length,
                         len(e_site),
                         2, 
                         len(rmf),
                         stat])
...
if mode == 1: # Approach #1
        ...
        e_fields = np.zeros([length, 
                             len(e_site),
                             2,
                             stat])
    
        comp_e_fields = np.zeros([length, 
                                  len(e_site),
                                  2, 
                                  stat])
...
for v1, ip1 in enumerate(e_site): # Sites where to calculate the e fields
    ...
    rmfb = femm.read_rmfb(secs_path + fname, 
                          ip1,  
                          storm
                          )  
    ...
    # 5.5.1.1) If the magnetics were recorded use the local tensor
        if str(e_site[v1]) in h_obs:    
            ...
            (loc_exmt, loc_eymt, 
             std_loc_exmt, std_loc_eymt) = femm.compute_e_fields(
                                         mh_obs[:,val,:],
                                         0*mh_obs[:,val,:], 
                                         tf_path, 
                                         ip1, 
            ...
            ax = np.array([loc_exmt]).T; ay = np.array([loc_eymt]).T; 
            a1 = np.array([std_loc_exmt]).T; a2 = np.array([std_loc_eymt]).T 
                         
            loc_emt [:,v1,:]= np.hstack((ax, ay))
            std_loc_emt [:,v1,:]= np.hstack((a1, a2))
            
            # Deffine the e vector using several values within the error bars
            for ip in range (0,stat):
                e_fields[:,v1,:, ip] = (loc_emt[:,v1,:] 
    ...
    if mode == 2:                       
        ...
        for v2, ip2 in enumerate(rmf): 
            ...
            (c_reg_ex, c_reg_ey, 
             c_std_reg_ex, c_std_reg_ey) = femm.compute_e_fields(
                    comp_rmf[:,v2,:],
                    std_comp_rmf[:,v2,:], 
                    tf_path, 
                    ip1, 
            ...
            cax = np.array([c_reg_ex]).T; cay = np.array([c_reg_ey]).T; 
            ca1 = np.array([c_std_reg_ex]).T; ca2 = np.array([c_std_reg_ey]).T
        ...
        for v3, ip3 in enumerate(rmf):
            ...
            for ip in range (0,stat):
                ...
                e_fields[:,v1,:,v3,ip] = (c_reg_e[:,v1,v3,:]
                                       + np.random.standard_normal([length,2])
                                       * c_std_reg_e[:,v1,v3,:])
                
                secs_e_fields[:,v1,:,ip] = ((secs_e[:,v1,:] 
                                        + np.random.standard_normal([length,2])
                                        * stdsecs_e[:,v1,:]))
                
                # Computed to create regional and local electric fields
                loc_e_fields[:,v1,:,ip] = ((secs_e[:,v1,:] 
                                        + np.random.standard_normal([length,2])
                                        * stdsecs_e[:,v1,:]))
                
                reg_e_fields[:,v1,:,v3,ip] = (c_reg_e[:,v1,v3,:]
                                        + np.random.standard_normal([length,2])
                                        * c_std_reg_e[:,v1,v3,:]) 
        ...
        #creg_e_fields = np.average(e_fields, axis = 3)
